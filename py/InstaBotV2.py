...
class InstaBot:
    ...
    def list_getList_1(self, processStep=70):
        ...
        # in actual life we need to mark them as drop if we open their page and it gives the
        ...
        myFollowers = [x for x in range(0, 729)]
        try:
        ...
        for user in theList_1_list:
            try:
                ...
                mark = 'keep'
                if dictio['followers'] > (1.05 * len(myFollowers)) or (dictio['posts'] <= 3):
                    mark = 'drop'
                    print('Dropping user: {0} has {1} followers and {2} posts'.format(user,
